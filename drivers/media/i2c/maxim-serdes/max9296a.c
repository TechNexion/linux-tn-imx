...
static int __maybe_unused max9296a_reg_read(struct max9296a_priv *priv, unsigned int reg,
					    unsigned int *val)
{
	int ret = 0;
...
static int __maybe_unused max9296a_reg_write(struct max9296a_priv *priv, unsigned int reg,
					     unsigned int val)
{
	dev_dbg(priv->dev, "%s(): reg [0x%04x], data [0x%02x]\n", __func__, reg, val);
...
static int __maybe_unused max9296a_set_bits(struct max9296a_priv *priv, unsigned int reg,
					     unsigned int bits)
{
...
static int __maybe_unused max9296a_update_bits(struct max9296a_priv *priv, unsigned int reg,
...
static int __maybe_unused max9296a_assign_bits(struct max9296a_priv *priv, unsigned int reg,
...
static int __maybe_unused max9296a_clear_bits(struct max9296a_priv *priv, unsigned int reg,
					     unsigned int bits)
{
	dev_dbg(priv->dev, "%s(): reg [0x%04x], bits [0x%02x]\n", __func__, reg, bits);
...
static int __maybe_unused max9296a_multi_reg_write(struct max9296a_priv *priv, const struct reg_sequence *regs,
					     int num)
{
	for (int i = 0; i < num; i++) {
...
static int max9296a_wait_for_device(struct max9296a_priv *priv)
{
	...
	err = read_poll_timeout(max9296a_reg_read, ret,
				!ret && val,
				100 * USEC_PER_MSEC,
				1 * USEC_PER_SEC, false,
				priv, MAX9296A_REG0, &val);
	if (err)
...
static int max9296a_reset(struct max9296a_priv *priv)
{
	...
	ret = max9296a_set_bits(priv, MAX9296A_CTRL0,
				    ...
				    struct max_des_pipe *pipe)
{
	...
	ret = max9296a_reg_read(priv, MAX9296A_VPRBS(index), &val);
	if (ret)
		return ret;
				   ...
				   struct max_des_phy *phy)
{
	...
	ret = max9296a_reg_read(priv, MAX9296A_MIPI_PHY18, &val);
	if (ret)
		return ret;
	...
	ret = max9296a_reg_read(priv, MAX9296A_MIPI_PHY20(index), &val);
	if (ret)
		return ret;
...
static int max9296a_set_enable(struct max_des *des, bool enable)
{
	...
	return max9296a_assign_bits(priv, MAX9296A_BACKTOP12,
...
static int max9296a_init_tpg(struct max_des *des)
{
	...
	return max9296a_multi_reg_write(priv, regs, ARRAY_SIZE(regs));
}
...
static int max9296a_init(struct max_des *des)
{
	...
	if (priv->info->rlms_adjust_sequence) {
		ret = max9296a_multi_reg_write(priv,
...
static int max9296a_init_phy(struct max_des *des, struct max_des_phy *phy)
{
	...
	ret = max9296a_assign_bits(priv, MAX9296A_MIPI_TX10(hw_index),
	...
	ret = max9296a_assign_bits(priv, MAX9296A_MIPI_PHY5(index),
	...
	ret = max9296a_clear_bits(priv, MAX9296A_DPLL_0(hw_index),
	...
	ret = max9296a_set_bits(priv, MAX9296A_BACKTOP22(hw_index),
			      MAX9296A_BACKTOP22_PHY_CSI_TX_DPLL_EN);
	if (ret)
		return ret;

	/* Pull DPLL block out of reset. */
	ret = max9296a_set_bits(priv, MAX9296A_DPLL_0(hw_index),
			      MAX9296A_DPLL_0_CONFIG_SOFT_RST_N);
	if (ret)
		return ret;
	...
	if (dpll_freq > 1500000000ull) {
		...
		ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX3(hw_index),
		...
		ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX4(hw_index),
		...
		ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX3(hw_index), 0x0);
		if (ret)
			return ret;

		/* Disable periodic deskew. */
		ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX4(hw_index), 0x0);
		if (ret)
			return ret;
	}
				 ...
				 struct max_des_phy_mode *mode)
{
	...
	ret = max9296a_assign_bits(priv, MAX9296A_MIPI_TX51(phy_id),
	...
	return max9296a_assign_bits(priv, MAX9296A_MIPI_TX51(phy_id),
				   ...
				   bool enable)
{
	...
	return max9296a_assign_bits(priv, MAX9296A_MIPI_PHY2,
				   ...
				   struct max_des_remap *remap)
{
	...
	ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX13(index, i),
	...
	/* TODO: implement extended Virtual Channel. */
	ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX14(index, i),
					   ...
					   unsigned int mask)
{
	...
	ret = max9296a_reg_write(priv, MAX9296A_MIPI_TX11(index), mask);
	if (ret)
		return ret;

	return max9296a_reg_write(priv, MAX9296A_MIPI_TX12(index), mask >> 8);
}
				    ...
				    bool enable)
{
	...
	return max9296a_assign_bits(priv, MAX9296A_REG2,
				    ...
				    bool enable)
{
	...
	return max9296a_assign_bits(priv, MAX9296A_VIDEO_PIPE_EN,
					   ...
					   struct max_des_pipe *pipe, bool enable)
{
	...
	return max9296a_assign_bits(priv, MAX9296A_MIPI_TX52(index),
				  ...
				  struct max_des_pipe_mode *mode)
{
	...
	ret = max9296a_assign_bits(priv, MAX9296A_BACKTOP21,
				 MAX9296A_BACKTOP21_BPP8DBL(index), mode->dbl8);
	if (ret)
		return ret;

	ret = max9296a_assign_bits(priv, MAX9296A_BACKTOP24,
	...
	ret = max9296a_assign_bits(priv, MAX9296A_BACKTOP32,
	...
	return max9296a_assign_bits(priv, MAX9296A_BACKTOP33,
...
static int max9296a_reset_link(struct max9296a_priv *priv, unsigned int index)
{
	...
	return max9296a_set_bits(priv, reg, mask);
}
				     ...
				     enum max_serdes_gmsl_version version)
{
	...
	ret = max9296a_update_bits(priv, reg, mask, field_prep(mask, val));
	if (ret)
		return ret;
	...
	ret = max9296a_assign_bits(priv, MAX9296A_MIPI_TX0(index),
				 MAX9296A_MIPI_TX0_RX_FEC_EN, gmsl3_en);
	if (ret)
		return ret;

	ret = max9296a_assign_bits(priv, MAX9296A_REG6,
				 MAX9296A_REG6_GMSL2_X(index), !gmsl3_en);
	if (ret)
		return ret;

	return max9296a_assign_bits(priv, MAX9296A_REG4,
				    ...
				    const struct max_serdes_tpg_timings *tm)
{
	...
	ret = max9296a_multi_reg_write(priv, regs, ARRAY_SIZE(regs));
	if (ret)
		return ret;

	return max9296a_reg_write(priv, MAX9296A_PATGEN_0,
...
static int max9296a_set_tpg_clk(struct max9296a_priv *priv, u32 clock)
{
	 ...
	 * pipe 0.
	 */
	ret = max9296a_update_bits(priv, MAX9296A_VPRBS(0),
	...
	return max9296a_update_bits(priv, MAX9296A_IO_CHK0,
...
static int max9296a_set_tpg_mode(struct max9296a_priv *priv, bool enable)
{
	...
	return max9296a_update_bits(priv, MAX9296A_PATGEN_1,
			    ...
			    const struct max_serdes_tpg_entry *entry)
{
	...
	return max9296a_assign_bits(priv, MAX9296A_MIPI_PHY0,
