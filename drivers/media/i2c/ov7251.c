...
/* Backporting V4L2 api from kernel version 4.17 */
...
struct ov7251_supported_res {
	u16 width;
	u16 height;
};

static const struct ov7251_supported_res supported_res[] = {
	{
		.width = 640,
		.height = 480,
	},
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->index >= ARRAY_SIZE(supported_res))
		return -EINVAL;

	fse->min_width = supported_res[fse->index].width;
	fse->max_width = supported_res[fse->index].width;
	fse->min_height = supported_res[fse->index].height;
	fse->max_height = supported_res[fse->index].height;
...
static int ov7251_probe(struct i2c_client *client)
{
	...
	if (IS_ERR(ov7251->power_gpio) || IS_ERR(ov7251->reset_gpio)) {
		dev_err(dev, "cannot get power or reset gpio. dts file is correct?\n");
		return -EINVAL;
	}
...
MODULE_AUTHOR("Leo Wang <leo.wang@technexion.com>");
