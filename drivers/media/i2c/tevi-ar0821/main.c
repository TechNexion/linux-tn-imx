...
static int sensor_load_bootdata(struct sensor *instance)
{
	...
	checksum = ar0821_otp_flash_get_checksum(instance->otp_flash_instance);

	//load pll
	bootdata_temp_area[0] = cpu_to_be16(BOOT_DATA_START_REG);
	pll_len = len = ar0821_otp_flash_get_pll_section(instance->otp_flash_instance,
	...
	len = ar0821_otp_flash_read(instance->otp_flash_instance,
				    (u8 *)(&bootdata_temp_area[1]),
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(dev, "data-lanes [%d] ,continuous-clock [%d]\n",
	...
	instance->otp_flash_instance = ar0821_otp_flash_init(dev);
	...
	sensor_i2c_write_16b(instance->i2c_client, 0x2000, 1920); //VIDEO_WIDTH
	sensor_i2c_write_16b(instance->i2c_client, 0x2002, 1080); //VIDEO_HEIGHT
...
MODULE_DESCRIPTION("TechNexion driver for TEVI-AR0821");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0");
MODULE_ALIAS("Camera");
