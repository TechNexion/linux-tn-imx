...
struct otp_flash *ar0821_otp_flash_init(struct device *dev)
{
	return NULL;
}

u16 ar0821_otp_flash_get_checksum(struct otp_flash *instance)
{
	return BOOTDATA_CHECKSUM;
}

size_t ar0821_otp_flash_read(struct otp_flash *instance, u8 *data, int addr, size_t len)
{
	size_t l;
...
size_t ar0821_otp_flash_get_pll_length(struct otp_flash *instance)
{
	return BOOTDATA_PLL_INIT_SIZE;
}

size_t ar0821_otp_flash_get_pll_section(struct otp_flash *instance, u8 *data)
{
	ar0821_otp_flash_read(instance, data, 0, BOOTDATA_PLL_INIT_SIZE);
	return BOOTDATA_PLL_INIT_SIZE;
}
#else
...
size_t ar0821_otp_flash_get_pll_section(struct otp_flash *instance, u8 *data)
{
	...
	if(temp[0] == 2) {
		...
		if (header->pll_bootdata_len != 0) {
			ar0821_otp_flash_read(instance, data, 0,
