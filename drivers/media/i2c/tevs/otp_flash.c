...
struct otp_flash *tevs_load_bootdata(struct i2c_client *client)
{
	...
	if (__header_ver == 2) {
		...
		dev_info(dev, "Product:%s, Version:%d. Lens:%s, Version:%d\n",
			 header->product_name, header->product_version,
			 header->lens_name, header->lens_version);
		...
		dev_info(
			...
			headerv3->product_name, headerv3->header_version,
			headerv3->tn_fw_version[0], headerv3->tn_fw_version[1],
			headerv3->vendor_fw_version, headerv3->custom_number,
