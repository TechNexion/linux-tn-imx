...
#define TEVS_FLICK_CTRL_FRC_OVERRIDE_UPPER_ET (3)
...
#define TEVS_FLICK_CTRL_MODE_50HZ \
	(TEVS_FLICK_CTRL_FREQ(50) | TEVS_FLICK_CTRL_MODE_MANUAL)
#define TEVS_FLICK_CTRL_MODE_60HZ \
...
#define TEVS_BACKLIGHT_COMPENSATION_MAX \
	HOST_COMMAND_ISP_CTRL_BACKLIGHT_COMPENSATION_MAX
#define TEVS_BACKLIGHT_COMPENSATION_MIN \
	HOST_COMMAND_ISP_CTRL_BACKLIGHT_COMPENSATION_MIN
...
int tevs_init_setting(struct tevs *tevs)
{
	...
	ret += tevs_i2c_write_16b(tevs, HOST_COMMAND_ISP_CTRL_PREVIEW_FORMAT,
				  0x50);
	ret += tevs_i2c_write_16b(tevs, HOST_COMMAND_ISP_CTRL_PREVIEW_HINF_CTRL,
...
static int tevs_get_exposure(struct tevs *tevs, s32 *value)
{
	...
	ret = tevs_i2c_read(tevs, TEVS_AE_MANUAL_EXP_TIME, val, 4);
	if (ret)
		return ret;
...
static int tevs_get_exposure_max(struct tevs *tevs, s64 *value)
{
	...
	ret = tevs_i2c_read(tevs, TEVS_AE_MANUAL_EXP_TIME_MAX, val, 4);
	if (ret)
		return ret;
...
static int tevs_get_exposure_min(struct tevs *tevs, s64 *value)
{
	...
	ret = tevs_i2c_read(tevs, TEVS_AE_MANUAL_EXP_TIME_MIN, val, 4);
	if (ret)
		return ret;
	*value = be32_to_cpup((__be32 *)val);
	return 0;
...
static int tevs_get_backlight_compensation_max(struct tevs *tevs, s64 *value)
{
	u16 val;
	int ret;
...
static int tevs_get_backlight_compensation_min(struct tevs *tevs, s64 *value)
{
	u16 val;
	int ret;
